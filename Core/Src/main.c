...
static int32_t hts221_write(void * handle, uint8_t reg,
  const uint8_t * bufp, uint16_t len);
    ...
    if (Flag_IRQ_Btn == 0) {
      ...
      //for(int i = 0; i < 10; i++)
      {
        ...
        if (f_mount( & SDFatFS, (TCHAR
          ...
          if (f_open( & file1, "LOG_Temp.TXT", FA_OPEN_APPEND | FA_WRITE) != FR_OK) {
            /* 'STM32.TXT' file Open for write Error */
            Error_Handler();
          } else {
            /*##-5- Write data to the text file ################################*/
            ...
            char * value = i;
            restest = f_write( & file1, & value, sizeof( & value), (void * ) & byteswritten);
            ...
            if ((byteswritten == 0) || (restest != FR_OK)) {
              ...
              if (f_open( & file1, "LOG_Temp.TXT", FA_READ) != FR_OK) {
                ...
                /*##-8- Read data from the text file ###########################*/
                restest = f_read( & file1, rtext, sizeof(rtext), (UINT * ) & bytesread);
...
void Display_LCD_Init() {
  ...
  BSP_LCD_DisplayStringAt(0, 10, (uint8_t * )
    "Menu Principal :", CENTER_MODE);
}
...
void Display_LCD_Button(int color) {
  if (color == 1) {
    ...
    if (refresh == 0) {
      ...
      BSP_LCD_SetTextColor(LCD_COLOR_WHITE);
      BSP_LCD_DisplayStringAt(60, BSP_LCD_GetYSize() - (1 * BSP_LCD_GetYSize() / 4), (uint8_t * )
        "REFRESH", LEFT_MODE);
      ...
      BSP_LCD_SetTextColor(LCD_COLOR_WHITE);
      BSP_LCD_DisplayStringAt(100, BSP_LCD_GetYSize() - (1 * BSP_LCD_GetYSize() / 4), (uint8_t * )
        "HOME", RIGHT_MODE);
    }

    BSP_LCD_DisplayStringAt(10, BSP_LCD_GetYSize() - (1 * BSP_LCD_GetYSize() / 2), (uint8_t * )
      "[         'C ]", CENTER_MODE);
    ...
    BSP_LCD_SetTextColor(LCD_COLOR_WHITE);
    BSP_LCD_DisplayStringAt(130, BSP_LCD_GetYSize() - (3 * BSP_LCD_GetYSize() / 4), (uint8_t * )
      "P1:Temp", LEFT_MODE);
    ...
    BSP_LCD_SetTextColor(LCD_COLOR_WHITE);
    BSP_LCD_DisplayStringAt(130, BSP_LCD_GetYSize() - (2 * BSP_LCD_GetYSize() / 4), (uint8_t * )
      "P2:Hum ", LEFT_MODE);
    ...
    BSP_LCD_SetTextColor(LCD_COLOR_WHITE);
    BSP_LCD_DisplayStringAt(130, BSP_LCD_GetYSize() - (1 * BSP_LCD_GetYSize() / 4), (uint8_t * )
      "P3:Pres", LEFT_MODE);
    ...
    BSP_LCD_SetTextColor(LCD_COLOR_WHITE);
    BSP_LCD_DisplayStringAt(330, BSP_LCD_GetYSize() - (3 * BSP_LCD_GetYSize() / 4), (uint8_t * )
      "P4:Vit", LEFT_MODE);
    ...
    BSP_LCD_SetTextColor(LCD_COLOR_WHITE);
    BSP_LCD_DisplayStringAt(330, BSP_LCD_GetYSize() - (2 * BSP_LCD_GetYSize() / 4), (uint8_t * )
      "P5:Dir", LEFT_MODE);
    ...
    BSP_LCD_SetTextColor(LCD_COLOR_WHITE);
    BSP_LCD_DisplayStringAt(330, BSP_LCD_GetYSize() - (1 * BSP_LCD_GetYSize() / 4), (uint8_t * )
      "P6:Pluv", LEFT_MODE);
