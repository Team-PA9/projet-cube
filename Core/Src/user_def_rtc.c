...
void RTC_Get_UTC_Timestamp(char *buffer) {
	...
	sprintf(buffer, "%02u-%02u-%04uT%02u:%02u:%02uZ", BCD_to_Dec(sDate1.Date),
			BCD_to_Dec(sDate1.Month), BCD_to_Dec(sDate1.Year) + 2000,
			BCD_to_Dec(sTime1.Hours), BCD_to_Dec(sTime1.Minutes),
			BCD_to_Dec(sTime1.Seconds));
}

void RTC_Get_Split(uint8_t *year, uint8_t *month, uint8_t *day, uint8_t *hours,
...
// To be tested
void RTC_Set(uint8_t hours, uint8_t minutes, uint8_t seconds, uint8_t day,
